...
export function partTwo(input = null) {
  ...
  function dependantBlocks(block) {
    const sandBlocksBeingHeldUp = sandBlocks
      .filter((b) => b[0][2] === block[1][2] + 1)
      .filter((b) => overlapsInXY(b, block));

    // Simple case: it's holding up none
    if (sandBlocksBeingHeldUp.length === 0) {
      return [];
    }

    // Get all blocks at the same height that are not falling
    const blocksAtSameHeight = sandBlocks.filter((v) => v[2] !== brickKey).filter((b) => b[1][2] === block[1][2]);

    console.log({ blocksAtSameHeight });

    if (!blocksAtSameHeight.length) {
      // First if there are no others at this height, its definitely all of the children!
      for (const sandBlock of sandBlocksBeingHeldUp) {
        sum += findChainFromBlock(sandBlock);
      }
      return sum; // We can end here as we've covered all the children
    }
  }

  function findChainFromBlock(block) {
    let sum = 0;
    let brickKey = block[2];
    fallingBlocks.add(brickKey);
    // Are there any this one is holding up?
    const sandBlocksBeingHeldUp = sandBlocks
      .filter((b) => b[0][2] === block[1][2] + 1)
      .filter((b) => overlapsInXY(b, block));
    ...
    // Get all blocks at the same height that are not falling
    const blocksAtSameHeight = sandBlocks
      .filter((v) => v[2] !== brickKey && fallingBlocks.has(v[2]))
      .filter((b) => b[1][2] === block[1][2]);
    ...
    // Otherwise, we have to determine the ones held up by non-falling blocks

    for (const blockBeingHeldUp of sandBlocksBeingHeldUp) {
      // If any of the blocks at the same
      if (!blocksAtSameHeight.some((b) => overlapsInXY(blockBeingHeldUp, b))) {
        sum += findChainFromBlock(blockBeingHeldUp);
      }
    }
